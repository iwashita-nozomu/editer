...
- `Ctrl+4`: focus the prompt window.
- `Ctrl+Tab`: cycle main -> files -> prompt -> main.
- `Up` / `Down` in prompt: move the output selection cursor.
- `Shift+Up` / `Shift+Down` in prompt: extend the output selection range.
- `Ctrl+C` in prompt: copy the selected prompt output range.
- `Ctrl+L` in prompt: clear prompt output.
...
- Prompt shells inherit `PATH` with the current Mado binary directory first, plus `MADO_HOST_BINARY`, `MADO_HOST_ROOT`, and `MADO_HOST_LAUNCH`.
- Docker CLI from the prompt can start with `docker compose exec <service> sh`; dedicated Docker child windows are planned.
