...
1. 問い、仮説、比較条件を固定する
1. `python3 scripts/experiments/create_experiment_topic.py <topic>` か手動更新で `experiments/registry.toml` に topic entry を固定する
...
main server host で実行する formal run では、`run_managed_experiment.py --use-registered-command formal` を経由して `run_manifest.json` と `run.log` を残します。
branch / worktree を使う場合は、scope 更新後に `sync_experiment_registry_context.py` で `active_branch` と `scope_file` を同期します。
...
python3 scripts/experiments/sync_experiment_registry_context.py --topic <topic> --branch work/<topic>-YYYYMMDD
