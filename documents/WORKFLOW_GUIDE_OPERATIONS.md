...
server を main 運用に据える前には、layout template と照合した readiness も残します。
...
## Main Server Readiness

main server host を固定したら、少なくとも次を記録します。

- host kind が Linux か WSL2 か
- bare repo root
- shared workspace root
- local state root
- Docker socket access の有無
- current shell が `docker` group を有効にしているか
- shell が stale なら `sg docker -c 'docker version'` で daemon 到達性を切り分けられること
