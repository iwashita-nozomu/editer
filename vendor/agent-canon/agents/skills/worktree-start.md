...
1. 以後の継続ログは `python3 tools/agent_tools/work_log.py --kind <kind> --status done --request-clause-id R1 --message "<what changed>" --next "<next>"` を既定にし、entry に `request_clause_ids=` と `status:` を残します。
...
- `python3 tools/agent_tools/work_log.py --kind edit --status done --request-clause-id R1 --message "..." --next "..."`
