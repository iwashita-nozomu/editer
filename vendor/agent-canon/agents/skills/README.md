...
- `memory/global/USER_PREFERENCES.md` の整理や `AGENTS.md` への昇格では `user-preference-sync` を使います。
- `memory/global/AGENT_PHILOSOPHY.md` と `memory/methods/` / `memory/candidates/` の更新や agent-side learning の整理では `agent-learning` を使います。
