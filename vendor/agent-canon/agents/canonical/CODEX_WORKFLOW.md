...
- parallel write-capable subagent を使う場合は、同じ file を重複割当てせず、same directory では file 単位の disjoint write scope を `schedule.md` に固定する
- file 単位で切れない場合は、同一 workspace の並列 write を避け、親が直列化するか別 worktree に分けて統合する
...
- same file を複数の write-capable subagent に同時に割り当てない
- same directory の parallel write は、disjoint file scope を明記できる場合だけにする
