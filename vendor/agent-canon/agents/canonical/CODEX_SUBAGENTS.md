...
| 包括的開発の統合レビュー | `project_reviewer` を intake / wrap-up で使い、必要に応じて `docs_workflow_steward` と `python_reviewer` を重ねる |
...
- 2 つの write-capable subagent に同じ file を同時に割り当てません
- 同じディレクトリを複数 worker が触る場合は、`schedule.md` に file 単位の disjoint write scope を書きます
- file 境界を切れない場合は、同一 workspace で並列 write をせず、parent が直列化するか別 worktree に分けます
- parent は worker の結果を 1 本ずつ統合し、未統合の parallel patch を抱えたまま次段へ進みません
...
- system 的に安全なのは、subagent ごとに forked workspace か明示的な write scope 分離を使う形です
- same directory を複数 subagent が触ること自体は許可できますが、同じ file の同時編集は許可しません
- same directory での parallel write は、親が file list を固定し、各 worker の write set が disjoint だと確認できる場合だけに限定します
- file 単位で切れない task は、worker を増やさず 1 agent にまとめるか、別 worktree に分けて統合 review を置きます
- review role は常に read-only とし、parallel write の安全性は `plan_reviewer`、`project_reviewer`、必要に応じて `critical_guardian -> project_reviewer` mapping で確認します
