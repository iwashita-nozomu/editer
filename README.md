...
- `scripts/`
  ...
  - `$start-repository` skill は `scripts/start_repository.sh` を呼び、その wrapper が clean clone では init 前の `make agent-canon-ensure-latest`、`scripts/init_from_template.sh`、`tools/update_agent_canon.sh register-local-bare`、必要な post-commit validation をまとめます。dirty state なら preflight の未実行理由を出します。
- `docker/`
