...
- `requirements_organizer`
  - 要件整理、acceptance criteria、reuse candidate 整理を行う
- `execution_planner`
  - 実行計画、stage 順序、担当 agent、validation 順序を固める
- `plan_reviewer`
  - 実行計画の順序、review 分離、rollback point を確認する
- `detailed_designer`
  - 詳細設計文書と reuse-first 方針の起草を行う
- `detailed_design_reviewer`
  - 詳細設計文書の十分性、既存コード踏襲、実装 readiness を確認する
- `explorer`
...
| ----- | ------------------------ |
| 要件整理 | `requirements_organizer`。既存 codebase survey が要るなら `explorer` も追加 |
| 調査 | `literature_researcher`。local precedent 調査が要るなら `explorer` |
| 実行計画立案 | `execution_planner` |
| 計画レビュー | `plan_reviewer` |
| 詳細設計 | `detailed_designer`。文書整備が中心なら `docs_workflow_steward` を補助に使う |
| 詳細設計レビュー | `detailed_design_reviewer`。repo-wide な影響が大きければ `project_reviewer` を追加 |
| 実装 | `worker` |
| 実装後レビュー | `reviewer`、Python 差分なら `python_reviewer` |

運用ルール:
- parent は、上の stage を 1 つの思考の中で暗黙にまとめません
- `plan_reviewer` と `detailed_design_reviewer` は同じ instance を再利用しません
- `detailed_design_reviewer` が unresolved のまま `worker` を起動しません
- `detailed_designer` と `worker` は、既存コードと既存の書き方を踏襲することを最優先にします
