...
| 文献調査や関連研究整理 | `literature_researcher`、必要なら `explorer` |
...
| 実験や調査つき改善 | 先行研究や外部根拠が重要なら `literature_researcher` か `explorer` で前提確認後、parent 実装。report があるなら `report_reviewer`。methodology や artifact policy が大きく動くなら perspective reviewers も使う。必要なら run bundle を作る |
